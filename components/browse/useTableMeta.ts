...
import { guessWidget, guessDisplayColumn, selectOptions, humanize, type Widget } from "@/lib/introspect/heuristics";
...
export interface ColumnMeta {
  ...
  ref: { connection: string; schema: string; table: string; column: string } | null;
  required: boolean;
}
...
export function buildTableMeta(
  ...
  const tOverride =
    catalog.tableOverrides.find(
      (o) => o.connectionId === conn.connectionId && o.schema === schema && o.table === tableName
    ) ?? null;
  const cOverrides = catalog.columnOverrides.filter(
    (o) => o.connectionId === conn.connectionId && o.schema === schema && o.table === tableName
  );
  const vfks = catalog.virtualFks.filter(
    (v) => v.fromConnection === connection && v.fromSchema === schema && v.fromTable === tableName
  );
  ...
  const columns: ColumnMeta[] = table.columns.map((col) => {
    ...
    const ref = realFk
      ? { connection, schema: realFk.referencedSchema, table: realFk.referencedTable, column: realFk.referencedColumns[0] }
      : vfk
        ? { connection: vfk.toConnection, schema: vfk.toSchema, table: vfk.toTable, column: vfk.toColumn }
        : null;
    return {
      col,
      ...
      readonly: (o?.readonly ?? false) || widget === "readonly" || baseWidget === "readonly",
  ...
  return {
    ...
    virtualFks: vfks,
    hasUpdatedAt: table.columns.some((c) => c.name === "updated_at"),
  };
}
