...
function Card({
  ...
  title: string;
  subtitle?: string;
  menu?: { label: string; onClick?: () => void; href?: string; danger?: boolean }[];
                  ...
                  {menu.map((m) =>
                    m.href ? (
                      <Link
                        ...
                        style={{ color: m.danger ? "var(--red)" : "var(--text)" }}
...
function FieldList({ meta, row, fkLabels }: { meta: TableMeta; row: Record<string, unknown>; fkLabels: Record<string, Record<string, string>> }) {
  const cols = meta.columns.filter((c) => !c.hidden && !["json", "jsonb"].includes(c.col.udtName));
  return (
      ...
      {cols.map((cm) => {
        ...
        return (
          <div key={cm.col.name} className="min-w-0">
            <div className="text-[11px] font-medium uppercase tracking-wide" style={{ color: "var(--text-faint)" }}>
              {cm.label}
            </div>
            <div className="text-[13px] truncate" style={{ color: f.muted ? "var(--text-faint)" : "var(--text)" }} title={f.text}>
              {label ? (
                <>
...
function JsonCard({ meta, row, pk, column }: { meta: TableMeta; row: Record<string, unknown>; pk: Record<string, unknown>; column: string }) {
    ...
    >
      {editing ? (
        <>
          ...
          <button className="btn btn-sm btn-primary mt-2" disabled={save.isPending} onClick={() => save.mutate()}>
...
function BelongsToCard({
  ...
  const [editing, setEditing] = useState(false);
  const { data, error } = useQuery<{ row: Record<string, unknown>; fkLabels: Record<string, Record<string, string>> }>({
  ...
  return (
    <Card
      ...
      menu={[
        ...
        { label: "Open table", href: `/browse/${target.connection}/${target.schema}/${target.table}` },
      ]}
    >
      {value == null ? (
        <p className="text-[13px]" style={{ color: "var(--text-faint)" }}>∅ not linked</p>
      ) : error ? (
        <p className="text-[13px]" style={{ color: "var(--red)" }}>{(error as Error).message}</p>
      ) : !data || !targetMeta ? (
        <div className="h-16 rounded animate-pulse" style={{ background: "var(--bg-raised)" }} />
      ) : (
...
function HasManyCard({
  ...
  const meta = useMemo(() => buildTableMeta(catalog, source.connection, source.schema, source.table), [catalog, source]);
  const [editingRow, setEditingRow] = useState<Record<string, unknown> | null>(null);
  const { data, error } = useQuery<{ rows: Record<string, unknown>[]; total: number | null; fkLabels: Record<string, Record<string, string>> }>({
  ...
  if (!meta) return null;
  const cols = meta.columns.filter((c) => !c.hidden && c.col.name !== fkColumn).slice(0, 4);
  return (
    <Card
      title={meta.label}
      subtitle={`${source.connection}.${source.schema}.${source.table} · via ${fkColumn}`}
      menu={[{ label: "Open table", href: `/browse/${source.connection}/${source.schema}/${source.table}` }]}
    >
      {error ? (
        <p className="text-[13px]" style={{ color: "var(--red)" }}>{(error as Error).message}</p>
      ) : !data ? (
        ...
        <p className="text-[13px]" style={{ color: "var(--text-faint)" }}>No related rows.</p>
      ) : (
        <>
            ...
            <table className="grid">
              ...
              <tbody>
                {data.rows.map((r, i) => {
                  ...
                  return (
                    <tr key={i} className="cursor-pointer" onClick={() => (window.location.href = href)}>
                      {cols.map((c) => {
                        const f = formatCell(r[c.col.name]);
                        const lbl = c.ref && r[c.col.name] != null ? data.fkLabels[c.col.name]?.[String(r[c.col.name])] : undefined;
                        return (
                          <td key={c.col.name} style={{ color: f.muted ? "var(--text-faint)" : undefined }}>
                            {lbl ?? f.text}
                          </td>
                        );
                      })}
...
function RecordView() {
  ...
  const { data, error } = useQuery<{ row: Record<string, unknown>; fkLabels: Record<string, Record<string, string>> }>({
    queryKey: ["record", params.connection, params.schema, params.table, JSON.stringify(pk)],
    queryFn: async () => {
      const res = await fetch(
  ...
  const relations = useMemo(() => {
    if (!catalog || !meta) return { belongsTo: [], hasMany: [] as { connection: string; schema: string; table: string; fkColumn: string }[] };
    const belongsTo = meta.columns
      .filter((c) => c.ref)
      .map((c) => ({ title: c.label, column: c.col.name, target: c.ref! }));
    const hasMany: { connection: string; schema: string; table: string; fkColumn: string }[] = [];
        ...
        for (const t of s.tables) {
          for (const fk of t.foreignKeys) {
            if (
              ...
              hasMany.push({ connection: conn.connectionName, schema: s.name, table: t.name, fkColumn: fk.columns[0] });
            }
          }
        }
      }
    ...
    for (const v of catalog.virtualFks) {
      if (v.toConnection === params.connection && v.toSchema === params.schema && v.toTable === params.table) {
        hasMany.push({ connection: v.fromConnection, schema: v.fromSchema, table: v.fromTable, fkColumn: v.fromColumn });
      }
    }
    return { belongsTo, hasMany };
  }, [catalog, meta, params]);

  if (!catalog || !meta) return <div className="px-8 py-10 text-[13px]" style={{ color: "var(--text-dim)" }}>Loading…</div>;
  if (error) return <div className="px-8 py-10 text-[13px]" style={{ color: "var(--red)" }}>{(error as Error).message}</div>;

  const row = data?.row;
  const jsonColumns = meta.columns.filter((c) => ["json", "jsonb"].includes(c.col.udtName) && !c.hidden);
  ...
  return (
    <div className="px-8 py-7 max-w-6xl">
      <div className="flex items-center gap-3 mb-5">
        <Link href={`/browse/${params.connection}/${params.schema}/${params.table}`} className="btn btn-sm">
          ← {meta.label}
        </Link>
          ...
          <>
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (res.ok) {
                  qc.invalidateQueries({ queryKey: ["rows", params.connection, params.schema, params.table] });
      ...
      {!row ? (
        <div className="h-32 rounded animate-pulse" style={{ background: "var(--bg-raised)" }} />
      ) : (
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <Card
              ...
              menu={meta.isView ? [] : [{ label: "✎ Edit record", onClick: () => setEditing(true) }]}
            >
...
export default function RecordPage() {
  return (
    <Suspense fallback={<div className="px-8 py-10 text-[13px]" style={{ color: "var(--text-dim)" }}>Loading…</div>}>
      <RecordView />
    </Suspense>
  );
}
