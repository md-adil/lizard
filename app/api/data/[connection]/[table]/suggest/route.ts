...
// Autocomplete suggestions: GET ?column=<column>&q=<search> — distinct
// existing values of that column, for the "autocomplete" widget.
export async function GET(req: Request, { params }: Params) {
  try {
    ...
    const options = await columnSuggestions(connection, schema, table, column, url.searchParams.get("q") ?? "");
    return ok(options);
  } catch (e) {
    return fail(e);
  }
